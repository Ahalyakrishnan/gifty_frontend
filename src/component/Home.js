...
const addcart=async(gift)=>{
  ...
  const fetchGifts = async () => {
    try {
      const response = await fetch('https://gifty-puze.onrender.com/gift/getAllGift'); 
  ...
  const handleCategory = async (categoryName) => {
    ...
    try {
      const response = await fetch(`https://gifty-puze.onrender.com/gift/searchcategory?category=${categoryName}`);
      if (response.ok) {
        const data = await response.json();
